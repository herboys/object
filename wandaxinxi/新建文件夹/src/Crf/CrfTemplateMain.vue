        ...
        <nav class="nav-top"
            ...
            <div class="left-tool">
                <i class="icon-home" @click="routertypebtn"
                   style="position: absolute;top: 13px;margin-top: 0px"></i><span style="margin-left: 30px"
                                                                                  class="check-text">{{CrfTemplateEditItems.templateName}}<i
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapState(['CrfTemplateEditItems', 'tempaltetypes', 'template_user_id']),
            listData() {
        ...
        methods: {
            ...
            ChildItemRouter(child) {
                ...
                if (child.questions.length > 0) {
                    ...
                    if (this.$route.query.preview === 2) {
                        console.log(child, '000000')
                        // if (child.questions[0].Answerlist !== undefined) {
                        console.log('我有答案')
                    ...
                    //当前item下面有组件问题=> preview==2的情况下（医生填报状态）
                    if (this.$route.query.preview == 4) {
                        this.FatherPassageway = child
                        console.log(child,'mian')
                        this.fistgetTempale()
                        this.hideType = false
