            ...
            <div class="search-bottom">
                ...
                <div class="condition-write">
                    <div class="write-top" ref="ele">
                        ...
                        <div class="div2 div" style="position:relative;width:100%;">
                            <p class="p1">
                            ...
                            <input class="in2" type="text" ref="initValue" id="initValue" placeholder="请输入检索值" style="text-indent:5px;" autocomplete="off"/>
                            <select class="se2">
                                <option>精确</option>
                                <option>模糊</option>
                            </select>
                            <input type="text" ref="table" id="table" v-model="collectionName" placeholder="请选择表名" disabled style="margin-left:10px;width:90px;text-indent:5px;">
                        ...
                        >
                            ...
                            <select class="se3" id="se3" ref="se3">
                             ...
                             <input type="text" id="" ref="collection" placeholder="请选择表名" disabled style="margin-left:10px;width:90px;text-indent:5px;">
...
export default {
  ...
  methods:{
      ...
      getItem(item){
          this.collectionName=item.value
      },
      addCondition(){            
            ...
            if(this.$refs.table.value == ""){
                ...
                let querystring = `${initValue}[${table}表 : ${init}]`
                 console.log(querystring,'0000000')
 ...
 //确定并开始搜索=====================================================================================
      startSearch(){
            ...
            if(this.$refs.table.value == ""){
                ...
                let table = this.$refs.table.value;
                let init = this.$refs.init.value;
                // this.querystring = initValue + table + table;
                ...
                this.ygmpara={
                    maplist:[{"检索字段":queryField,"运算符":computeValue,"检索值":initValue}]
                }
                axios({
                    url:'api/senior/gjsearch',
                    method:'post',
                    data:this.ygmpara
                }).then(res => {
                    // console.log(res)
                    this.seniorSearchData = res.data;
                    localStorage.setItem('searchresult',JSON.stringify( this.seniorSearchData))
                    localStorage.setItem('count',JSON.stringify(res.data.count))
                    // console.log(this.seniorSearchData)
                    if(res.status == 200){
                          this.$router.push({path:'/searchResult',query:{name:'advanced',data:this.seniorSearchData,value:query}})      
                        loading.close();   
                    }
                    if(res.status == 204){
                        loading.close();  
                ...
                // let query = initValue + arr1[0].value;
                let query = [initValue,arr1[0].value];
                // console.log(query)
                ...
                // let query = initValue + arr1[0].value + arr1[1].value;
                let query = [initValue,arr1[0].value,arr1[1].value];
                // let querystring = `${initValue}[${table}表 : ${init}]`
                //没输入字符警告
                this.warning(this.$refs.collection,this.$refs.box,this.$refs.boxValue)
                ...
                // let query = initValue + arr1[0].value + arr1[1].value +arr1[2].value;
                let query = [initValue , arr1[0].value , arr1[1].value , arr1[2].value];
                // let querystring = `${initValue}[${table}表 : ${init}]`
                //没输入字符警告
                this.warning(this.$refs.collection,this.$refs.box,this.$refs.boxValue)
                ...
                // let query = initValue + arr1[0].value + arr1[1].value + arr1[2].value + arr1[3].value;
                let query = [initValue , arr1[0].value , arr1[1].value , arr1[2].value , arr1[3].value];
                // let querystring = `${initValue}[${table}表 : ${init}]`
                //没输入字符警告
                this.warning(this.$refs.collection,this.$refs.box,this.$refs.boxValue);
  ...
  mounted(){
       this.$refs.saveCondition.innerHTML = localStorage.getItem("searchCondition")
        this.$refs.table.value = this.value
  }
}
</script>
